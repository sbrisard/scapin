...
auto make_tuple_from_std_array(std::array<T, SIZE> a) {
  ...
  if constexpr (SIZE == 0) return py::make_tuple();
  else if constexpr (SIZE == 1) return py::make_tuple(a[1]);
  else if constexpr (SIZE == 2) return py::make_tuple(a[0], a[1]);
  else if constexpr (SIZE == 3) return py::make_tuple(a[0], a[1], a[2]);
  else if constexpr (SIZE == 4) return py::make_tuple(a[0], a[1], a[2], a[3]);
}
