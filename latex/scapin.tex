...
\item \(\Omega\): the \(d\)-dimensional unit-cell
\item \(L_1,\ldots, L_d\): the dimensions of the unit-cell:
  \(\Omega=(0, L_1)\times(0, L_2)\times\cdots\times(0, L_d)\)
\item \(\lvert\Omega\rvert=L_1L_2\cdots L_d\): the volume of the unit-cell
\item \(\tuple{n}\): \(d\)-dimensional tuple of integers
  \(\tuple{n}=\bigl(n_1, n_2, \ldots, n_d\bigr)\)
\end{itemize}
...
\section{The simulation grid}

We first define the grid that defines the simulation. The unit-cell
\(\Omega=(0, L_1)\times\cdots\times(0, L_d)\) is divided in a regular grid of
size \(N_1\times\cdots\times N_d\). The cells of this grid are
\begin{equation}
  \Omega_{\tuple{p}}=\{p_ih_i\leq x_i<\bigl(p_i+1\bigr)h_i, i=1,\ldots, d\},
  \qquad\text{\emph{(no summation on \(i\)).}}
\end{equation}
where \(\tuple{p}\) denotes a \(d\)-tuple of integers,
\(\tuple{p}=\bigl(p_1,\ldots,p_d\bigr)\) such that \(0\leq p_i<N_i\),
\(i=1,\ldots, d\) and \(h_i=L_i/N_i\) is the cell-size in the \(i\)-th
direction. Note that in the above definition of \(\Omega_p\), no summation is
performed on the repeated index \(i\). The total number of cells is
\(N=N_1\cdots N_d\).

We consider discrete stress-polarizations \(\tens\tau^h\) that are constant
over each cell of the grid: \(\tens\tau_{\tuple{p}}^h\) denotes the constant
value of \(\tens\tau^h\) in cell \(\Omega_{\tuple{p}}\). The \(h\) superscript
reminds that \(\tens\tau^h\) is a discrete approximation of the true
stress-polarization. In order to solve the Lippmann--Schwinger equation, we
