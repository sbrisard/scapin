...
\section{Discrete Green operators}

In this section, we introduce various discretizations of the Green operator~;
we will adopt the vocabulary of linear elasticity, ,although the concepts apply
to all the types of physics presented in Sec.~\ref{sec:sec:202002060528}. We
first define the grid that defines the simulation. The unit-cell
\((0, L_1)\times\cdots\times(0, L_d)\) is divided in a regular grid of size
\(N_1\times\cdot\times N_d\). We consider discrete stress-polarizations
\(\tens\tau^h\) that are constant over each cell of the grid~; \(\tens\tau_n\)
denotes the constant value of \(\tens\tau^h\) in cell \(\Omega_n\) \todo{define
  this cell}. We want to approximate the true value of
\(\tens\Gamma_0(\tens\tau^h)\) as follows
\begin{equation}
  \tens\Gamma_0
\end{equation}
\printbibliography
